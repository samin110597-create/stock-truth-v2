...
const benchmarks={};
const panelIds=['decision','evidence','trade-matrix','thesis','mtf','reversal','patterns','technicals','levels','fundamentals','valuation','catalysts','horizons','validation','ledger','health'];
...
function fundamentalPanels(){
  const f=state.fundamentals,metrics=f?.metrics||{},fact=k=>metrics[k]?.latest;
  if(!f?.metrics)$('#fundamentals').innerHTML=`<h2>Fundamentals</h2>${pill('FUNDAMENTALS UNAVAILABLE')}<p class="note">${esc(f?.reason||f?.latest_attempt_error||'No source returned filing facts for this ticker.')}</p>`;
  else $('#fundamentals').innerHTML=`<h2>Reported financials <span class="tag">SEC FILING FACTS</span></h2><p class="note">${esc(f.provider)} · fetched ${when(f.fetched_at)} ${f.status==='STALE'?'· STALE':''}</p>${table(['Metric','Value','Period','Filed','Type'],Object.entries(metrics).map(([k,v])=>{const r=v.latest;return row(esc(k.replaceAll('_',' ')),r?.unit==='ratio'?pct(r.value):r?.unit==='USD/shares'?money(r.value):compact(r?.value),esc(r?.period_end||'Unavailable'),esc(r?.filed||'Unavailable'),pill(v.classification));}))}<p class="note">Annual flows are not TTM. Filing dates are preserved; these present-day facts are excluded from price backtests. Debt remains the exact reported component.</p>`;
  const q=state.quote?.price,eps=fact('eps_diluted'),shares=fact('shares_outstanding');
  const splitAfter=raw.timeframes?.['1D']?.splits?.some(s=>s.ts>Date.parse(eps?.period_end||'')/1000);
  const shareBasisKnown=Array.isArray(raw.timeframes?.['1D']?.splits)&&raw.timeframes['1D'].adjustment?.includes('split-adjusted');
  const pe=eps&&eps.value>0&&finite(q)&&shareBasisKnown&&!splitAfter?q/eps.value:null;
  $('#valuation').innerHTML=`<h2>Growth, profitability & valuation context</h2><div class="metrics">${metric('Annual revenue growth',pct(fact('revenue_growth')?.value),'Reported annual periods')}${metric('Operating margin',pct(fact('operating_margin')?.value),'Matching reporting periods')}${metric('Annual free cash flow',compact(fact('free_cash_flow')?.value),'Operating cash flow less capex')}${metric('Price / annual EPS',finite(pe)?fmt(pe)+'×':'—','Not TTM; review share basis')}${metric('Filed shares',compact(shares?.value),shares?.period_end||'Unavailable')}${metric('Cash',compact(fact('cash')?.value),fact('cash')?.period_end||'Unavailable')}${metric('Reported debt component',compact(fact('long_term_debt')?.value),fact('long_term_debt')?.tag||'Unavailable')}${metric('ROIC / intrinsic value','Unavailable','No imputed tax rate or valuation assumptions')}</div><p class="note">No price target is inferred from missing fundamentals. The technical trade plan remains independent. Annual EPS multiples are current-price comparisons with the labeled filed period and require a consistent share basis.</p>`;
  $('#catalysts').innerHTML=`<h2>Catalysts, analyst revisions & positioning</h2>${pill('UNAVAILABLE')}<p class="note">No current sourced earnings calendar, analyst revisions, news feed, options flow or insider-transaction feed was returned. These inputs contribute no evidence to the verdict.</p>`;
}
...
async function load(symbol){
  ...
  try{
    ...
    worker.onmessage=event=>{if(event.data.id!==requestId)return;if(event.data.error){clear(symbol,'Analysis unavailable');$('#load-status').textContent=event.data.error;return;}
      state=event.data.analysis;remember();const errors=render();$('#load-status').textContent=errors.length?'Panel unavailable: '+errors.join('; '):`${symbol} · ${raw.retrieval} · calculated ${when(state.generated_at)} · completed-bar signals`;
...
$('#brief').onclick=()=>{if(!state)return;const p=plan(),blob=new Blob([JSON.stringify({symbol:state.symbol,model:state.model_version,source_timestamp:state.source_data_timestamp,quote:state.quote,read:state.read,thesis:state.thesis,plan:p,validation:state.validation[selected().key],data_health:state.health},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=state.symbol+'-stock-truth-research.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
async function boot(){
  selectView('verdict');
  try{
    ...
    $('#build').textContent=build?build.model_version+' · '+build.commit.slice(0,8):'Build unavailable';
