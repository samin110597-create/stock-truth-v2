...
function waitDetail(){
  const d=state?.frames?.['1D'],read=state?.read||{},s=d?.structure||{},t=d?.technicals||{},r=d?.reversal||{};
  const support=s.support?.[0]?.price,resistance=s.resistance?.[0]?.price;
  const trend=t.intermediate_trend||'UNAVAILABLE',pattern=s.pattern||'UNAVAILABLE';
  const reversal=r?.dir?(r.dir>0?'bullish ':'bearish ')+(r.stage||'reversal'):(r.stage||'no confirmed reversal');
  const why='Current daily state: '+trend.toLowerCase()+' trend · '+pattern.toLowerCase()+' structure · '+reversal.toLowerCase()+
    (finite(t.rsi)?' · RSI '+fmt(t.rsi,1):'')+'. No fresh confirmed entry currently satisfies the selected setup rules.';
  const bull=finite(resistance)?'Bull trigger: completed close above '+money(resistance)+' followed by a successful retest.':'Bull trigger: wait for a new confirmed resistance/BOS level.';
  const bear=finite(support)?'Bear trigger: completed break below '+money(support)+' followed by a failed reclaim/retest.':'Bear trigger: wait for a new confirmed support/CHoCH level.';
  return {why,bull,bear,read:read.label||'UNAVAILABLE'};
}
function verdict(){
