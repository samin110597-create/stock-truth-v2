...
async function run(){
  ...
  try{const asset=detectAsset(symbol,$('#asset').value),timeframe=$('#tf').value,data=await loadMarketData({symbol,asset,timeframe,signal:controller.signal}),q=analyzeQuant({...data});q.apiContext=data.apiContext;q.dataStatus=data.dataStatus||'COMPLETED BAR';q.lastCompletedBar=data.lastCompletedBar||data.bars?.at(-1)?.end_ts||null;render(q);$('#status').textContent=`READY · ${data.provider} · ${data.bars.length.toLocaleString()} completed bars · ${q.state.probabilityStatus}`;history.replaceState(null,'',`?symbol=${encodeURIComponent(symbol)}&tf=${timeframe}&asset=${asset}`);}
