...
function drawProjection(q){
  ...
  const mc=q.forecast.monteCarlo;if(!mc?.length)return;
  const sets=[{key:'median',color:'#49d7e6',width:2},{key:'p25',color:'#7f93a3',width:1},{key:'p75',color:'#7f93a3',width:1}];
...
function render(q){
  const k=klass(q.state.direction),p=q.plan,pr=q.forecast.projection,h=q.forecast.historical,m=q.math,s=q.structure,ctx=q.apiContext||null,h10=q.trained?.horizons?.['10']||null;
  ...
  const probText=q.state.probabilityStatus==='WALK_FORWARD_VALIDATED'?pct(q.state.calibratedProbabilityDirection):'WITHHELD';
  $('#decision').innerHTML=`<div>${cell('MODEL ACTION',esc(q.state.action),k)}<div class="micro">${esc(p?.detail||'No trade until the model develops an edge.')}</div></div>${cell('DIRECTION',esc(q.state.direction),k)}${cell('10-BAR P(DIRECTION)',probText,q.state.probabilityStatus==='WALK_FORWARD_VALIDATED'?k:'amber')}${cell('REGIME',esc(q.state.regime))}${cell('QUALITY',esc(q.state.quality),q.state.quality==='A'?'up':'amber')}`;
  $('#source').textContent=q.provider;
  $('#trade').innerHTML=`<h2>EXECUTION MAP</h2><div class="trade-action ${k}">${esc(q.state.action)}</div>${p?kv('Bias',p.direction,k)+kv('10-bar calibrated P(direction)',probText,q.state.probabilityStatus==='WALK_FORWARD_VALIDATED'?k:'amber')+kv('Preferred entry',money(p.entryZone.low)+' – '+money(p.entryZone.high),'amber')+kv(p.direction==='LONG'?'Breakout trigger':'Breakdown trigger',money(p.trigger),'cyan')+kv('Fixed invalidation',money(p.stop),'down')+p.targets.map(t=>kv(t.name,money(t.price)+' · '+(finite(t.riskReward)?num(t.riskReward)+'R':'—')+' · '+esc(t.source),'up')).join('')+kv('Nearest target R:R',finite(p.rr)?num(p.rr)+'R':'—')+`<div class="rule"><b>Entry condition:</b> ${esc(p.entryRule)}<br><br><b>Failure:</b> ${esc(p.invalidation)}</div>`:'<div class="rule">No actionable execution map while the model is neutral.</div>'}`;
  ...
  const skill=h10?.metrics?.brier_skill;
  $('#state-grid').innerHTML=
    ...
    stateCard('OOS Brier skill',finite(skill)?pct(skill):'WITHHELD',h10?.validated?`${h10.oosSamples||0} out-of-sample observations`:esc(h10?.status||'No trained model'),finite(skill)?Math.max(0,Math.min(100,skill*1000)):null)+
  ...
  const trainedEvidence=h10?`${h10.validated?'PROMOTED':'WITHHELD'} · OOS ${h10.oosSamples||0} · Brier ${num(h10.metrics?.brier,4)} vs base ${num(h10.metrics?.base_brier,4)} · skill ${finite(skill)?pct(skill):'—'} · log loss ${num(h10.metrics?.log_loss,4)} vs base ${num(h10.metrics?.base_log_loss,4)}`:'No Q2 trained model artifact available.';
  ...
  $('#integrity').innerHTML=kv('Engine',esc(q.model))+kv('Trained artifact',esc(q.trained?.modelVersion||'Unavailable'))+kv('Trained artifact generated',q.trained?.generatedAt?new Date(q.trained.generatedAt).toLocaleString():'—')+kv('Probability policy',esc(q.state.probabilityStatus))+kv('Projection source',esc(q.forecast.source||'—'))+kv('Requested',esc(q.symbol))+kv('Source symbol',esc(q.sourceSymbol))+kv('Bars',q.bars.length.toLocaleString())+kv('Primary bar provider',esc(q.provider))+kv('Fetched',q.fetchedAt?new Date(q.fetchedAt).toLocaleString():'—')+kv('Credential handling',esc(q.credentialPolicy||'No browser credential'))+apiLines+`<div class="micro">${esc(ctx?.purpose||'API context is informational and does not silently change the trade score.')} ${esc(q.caveats.join(' '))}</div>`;
  renderChart(q);
}
async function run(){
  const symbol=$('#symbol').value.trim().toUpperCase();if(!symbol)return;controller?.abort();controller=new AbortController();$('#status').textContent='Loading secured data, calibrated model and Q-State 2.0…';
  try{const asset=detectAsset(symbol,$('#asset').value),timeframe=$('#tf').value,data=await loadMarketData({symbol,asset,timeframe,signal:controller.signal}),q=analyzeQuant({...data});q.apiContext=data.apiContext;render(q);$('#status').textContent=`READY · ${data.provider} · ${data.bars.length.toLocaleString()} completed bars · ${q.state.probabilityStatus}`;history.replaceState(null,'',`?symbol=${encodeURIComponent(symbol)}&tf=${timeframe}&asset=${asset}`);}
