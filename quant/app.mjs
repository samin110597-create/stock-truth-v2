...
function render(q){
  const k=klass(q.state.direction),p=q.plan,pr=q.forecast.projection,h=q.forecast.historical,m=q.math,s=q.structure,ctx=q.apiContext||null;
  ...
  $('#integrity').innerHTML=kv('Model',esc(q.model))+kv('Requested',esc(q.symbol))+kv('Source symbol',esc(q.sourceSymbol))+kv('Bars',q.bars.length.toLocaleString())+kv('Primary bar provider',esc(q.provider))+kv('Fetched',q.fetchedAt?new Date(q.fetchedAt).toLocaleString():'—')+kv('Credential handling',esc(q.credentialPolicy||'No browser credential'))+apiLines+`<div class="micro">${esc(ctx?.purpose||'API context is informational and does not silently change the trade score.')} ${esc(q.caveats.join(' '))}</div>`;
  renderChart(q);
}
async function run(){
