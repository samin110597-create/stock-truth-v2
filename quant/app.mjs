...
function render(q){
  const k=klass(q.state.direction),p=q.plan,pr=q.forecast.projection,h=q.forecast.historical,m=q.math,s=q.structure,ctx=q.apiContext||null,primaryH=String(q.state.calibratedProbabilityHorizon||({'15M':20,'1H':10,'4H':5,'1D':10}[q.timeframe]||10)),hPrimary=q.trained?.horizons?.[primaryH]||null;
