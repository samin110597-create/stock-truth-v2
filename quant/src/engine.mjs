...
function trainedProjection(trained,price,stop,dir){
  const hs=trained?.horizons||{},h5=hs['5'],h10=hs['10'],h20=hs['20'];
  if(!h5?.validated||!h10?.validated||!h20?.validated||!h5.returnBand||!h10.returnBand||!h20.returnBand)return null;
  const primary={label:dir>0?'CALIBRATED UPSIDE PATH':'CALIBRATED DOWNSIDE PATH',bar5:bandPrice(price,h5.returnBand,'q50'),bar10:bandPrice(price,h10.returnBand,'q50'),bar20:bandPrice(price,h20.returnBand,'q50')};
  const pullback={label:dir>0?'CALIBRATED PULLBACK PATH':'CALIBRATED BOUNCE PATH',bar5:bandPrice(price,h5.returnBand,dir>0?'q25':'q75'),bar10:bandPrice(price,h10.returnBand,dir>0?'q25':'q75'),bar20:bandPrice(price,h20.returnBand,dir>0?'q25':'q75')};
  const expansion={label:'CALIBRATED EXPANSION PATH',bar5:bandPrice(price,h5.returnBand,dir>0?'q75':'q25'),bar10:bandPrice(price,h10.returnBand,dir>0?'q75':'q25'),bar20:bandPrice(price,h20.returnBand,dir>0?'q75':'q25')};
  return {source:'WALK_FORWARD_OOS_CONDITIONAL_RETURNS',primary,pullback,expansion,failure:{price:stop,label:'THESIS FAILURE'},range10:{low:bandPrice(price,h10.returnBand,'q25'),high:bandPrice(price,h10.returnBand,'q75'),tailLow:bandPrice(price,h10.returnBand,'q10'),tailHigh:bandPrice(price,h10.returnBand,'q90')},range20:{low:bandPrice(price,h20.returnBand,'q25'),high:bandPrice(price,h20.returnBand,'q75'),tailLow:bandPrice(price,h20.returnBand,'q10'),tailHigh:bandPrice(price,h20.returnBand,'q90')}};
}
...
export function analyzeQuant({symbol,sourceSymbol=symbol,asset='STOCK',timeframe='1D',bars,mtf={},trainedModel=null,provider='unknown',fetchedAt=null,credentialPolicy=null}){
  ...
  const h10=trained?.horizons?.['10'],calibrated=!!h10?.validated&&finite(h10.probabilityUp),modelEdge=calibrated?2*(h10.probabilityUp-.5):null;
  ...
  const pDir=calibrated&&dir?(dir>0?h10.probabilityUp:1-h10.probabilityUp):null;let stage='NO EDGE',action='NO TRADE — EDGE TOO WEAK',detail='Wait for cleaner structure, better reward/risk, or stronger directional evidence.';
  if(dir){
    if(calibrated){
  ...
  const historical=historicalContext(clean,dir,10),skill=h10?.metrics?.brier_skill,quality=stage==='READY'?(calibrated&&pDir>=.62&&finite(skill)&&skill>=.02&&(!finite(alignment)||alignment>=.25)?'A':'B+'):stage==='DEVELOPING'?'B':'WATCH';
  ...
  const proj=dir?(trainedProjection(trained,price,stop,dir)||fallbackProjection(mc,dir,stop)):null;
  return {model:'Q-STATE 2.0',symbol,sourceSymbol,asset,timeframe,provider,fetchedAt,credentialPolicy,bars:clean,generatedAt:new Date().toISOString(),state:{direction:dir>0?'BULLISH':dir<0?'BEARISH':'NEUTRAL',score,heuristicScore:heuristic,conviction,stage,regime,action,quality,calibratedProbabilityUp:calibrated?h10.probabilityUp:null,calibratedProbabilityDirection:pDir,probabilityStatus:calibrated?'WALK_FORWARD_VALIDATED':h10?.status||'WITHHELD'},structure,mtf:{...mtfState,alignmentWithTrade:alignment},trained,indicators:{ema20:e20[last],ema50:e50[last],ema200:e200[last],rsi:rsi14[last]},math:{velocity,acceleration,jerk:jerkNow,curvature,entropy,hurst,dominantCycle:cycle,realizedVol:rv,parkinsonVol:pv,compression,volumeZ,bullAbsorption,bearAbsorption},forecast:{monteCarlo:mc?.steps||[],historical,projection:proj,source:proj?.source||null},plan,caveats:['Displayed numerical probability is withheld unless the walk-forward model beats its base-rate Brier benchmark and passes the promotion gate.','Walk-forward return bands use OOS predictions only when validated; otherwise projections fall back to a regime/volatility simulation.','Targets combine major confirmed structure with validated conditional return bands when available.','Absorption remains an OHLCV proxy unless true order-flow data is supplied.']};
}
