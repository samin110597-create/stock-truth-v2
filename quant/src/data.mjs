...
async function backendMarket(symbol,tf,signal){
  ...
  if(!base)throw new Error('ON-DEMAND BACKEND OFF: Deno API URL is not configured');
  const u=new URL(base+'/v1/market');
  u.searchParams.set('symbol',symbol);
  u.searchParams.set('timeframe',tf);
  const j=await json(u.toString(),signal,60000);
  if(j?.error)throw new Error(j.error+(j.message?' · '+j.message:''));
  const b=j?.timeframes?.[tf]||j?.primary;
  if(!b||!Array.isArray(b.bars)||b.bars.length<80)throw new Error('Deno API returned insufficient '+tf+' data');
...
export async function loadMarketData({symbol,asset='AUTO',timeframe='1D',signal}){
  ...
  try{core=await backendMarket(s,timeframe,signal);}
    ...
    backendError=e;
    if(kind==='FUTURE'){
      try{core=await future(s,timeframe,signal);}catch(fe){throw new Error(s+' data unavailable. Deno API: '+(backendError?.message||'failed')+' · futures fallback: '+(fe?.message||'failed'));}
    }else{
      try{core=await storedStock(s,timeframe,signal);}catch(se){storedError=se;try{core=await publicStock(s,timeframe,signal);}catch(pub){throw new Error(s+' data unavailable. Deno API: '+(backendError?.message||'failed')+' · stored snapshot: '+(storedError?.message||'failed')+' · public fallback: '+(pub?.message||'failed'));}}
    }
  }
