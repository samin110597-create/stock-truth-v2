...
if(command==='restore'){
  ...
  try{git(['fetch','origin',branch,'--depth=1']);git(['worktree','add','--detach',store,'FETCH_HEAD']);for(const name of ['raw','analysis','index.json','ledger.json','collection.json','validation-report.json'])if(fs.existsSync(path.join(store,name)))fs.cpSync(path.join(store,name),path.join('data',name),{recursive:true});console.log('Restored latest optional snapshots.');}
  ...
  for(const name of ['raw','analysis','index.json','ledger.json','collection.json','validation-report.json'])if(fs.existsSync(path.join('data',name)))fs.cpSync(path.join('data',name),path.join(store,name),{recursive:true});
  git(['add','raw','analysis','index.json','ledger.json','collection.json','validation-report.json'],store);
