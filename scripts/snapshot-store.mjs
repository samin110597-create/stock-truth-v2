...
if(command==='restore'){
  ...
  for(const name of ['raw','fundamentals','analysis','quant','index.json','ledger.json','collection.json','validation-report.json'])if(fs.existsSync(path.join('data',name)))fs.cpSync(path.join('data',name),path.join(store,name),{recursive:true});
  const publishable=['raw','fundamentals','analysis','quant','index.json','ledger.json','collection.json','validation-report.json'].filter(name=>fs.existsSync(path.join(store,name)));if(publishable.length)git(['add',...publishable],store);
