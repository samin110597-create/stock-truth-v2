...
export function reversals(b,tech,st){
  ...
  for(let i=0;i<b.length;i++){
    ...
    const recent=s.recentEvents,upSweep=recent.filter(e=>e.dir>0&&e.type==='LIQUIDITY SWEEP').at(-1),downSweep=recent.filter(e=>e.dir<0&&e.type==='LIQUIDITY SWEEP').at(-1);
    const proxies={absorption:false,distribution:false,capitulation:false,buying_climax:false};
    ...
    out.push({classification:'PROXY',...selected,stage:conflict?'CONFLICTING REVERSAL EVIDENCE':selected.stage,dir:conflict?0:selected.dir,proxies,sides});
  }
  ...
  return {states:out,current:last||null};
}
