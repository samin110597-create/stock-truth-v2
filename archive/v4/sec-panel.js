(()=>{
  ...
  let last='';
  function addAuditCaveat(){
    const t=document.querySelector('#truth');if(!t||t.dataset.overlapNote)return;
    t.dataset.overlapNote='1';
    t.insertAdjacentHTML('beforeend','<br><br><b>Statistical caveat:</b> 5-session and 21-session forward-return windows can overlap, so row counts are not the same as fully independent market episodes. Win rate and Brier skill remain useful descriptive OOS metrics, but confidence should be treated conservatively; model promotion requires multiple gates rather than one percentage.');
  }
  ...
  function detect(){addAuditCaveat();const t=document.querySelector('#title')?.textContent||'',m=t.match(/\(([^()]+)\)\s*$/);if(m)load(m[1]);}
  window.addEventListener('DOMContentLoaded',()=>{const title=document.querySelector('#title');if(title)new MutationObserver(detect).observe(title,{childList:true,subtree:true,characterData:true});const truth=document.querySelector('#truth');if(truth)new MutationObserver(addAuditCaveat).observe(truth,{childList:true,subtree:true,characterData:true});detect();});
})();