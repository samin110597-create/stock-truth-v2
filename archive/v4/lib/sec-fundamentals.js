const UA=process.env.SEC_USER_AGENT||'StockTruthV2/5.2 samin110597-create@users.noreply.github.com';
...
async function get(url){
  ...
  for(let attempt=0;attempt<3;attempt++){
    const u=new URL(url);
    const r=await fetch(url,{headers:{'User-Agent':UA,'Accept':'application/json','Accept-Encoding':'gzip, deflate','Host':u.host},cache:'no-store'});
    if(r.ok)return r.json();
    last=new Error(`SEC returned HTTP ${r.status}`);
    if(![403,429,500,502,503,504].includes(r.status)||attempt===2)throw last;
    await sleep(350*(attempt+1));
  }
  throw last||new Error('SEC request failed');
}
async function tickerMap(){
  if(TICKER_MAP)return TICKER_MAP;
  TICKER_MAP=await get('https://www.sec.gov/files/company_tickers.json');
  return TICKER_MAP;
}
...
async function secFundamentals(symbol,cikHint=null){
  let hit=null;
  if(cikHint&&/^\d{1,10}$/.test(String(cikHint)))hit={cik_str:Number(cikHint),title:symbol};
  if(!hit){
    const map=await tickerMap();
    for(const v of Object.values(map||{})){if(normTicker(v?.ticker)===normTicker(symbol)){hit=v;break;}}
  }
  if(!hit)return{source:'SEC EDGAR companyfacts',error:'Ticker is not mapped in SEC company facts. Non-U.S. securities, funds, and some newly listed issuers may not have usable company facts.'};
  ...
  const taxRate=pretax&&tax!=null?Math.min(.5,Math.max(0,tax/pretax)):.21,invested=(equity||0)+(debt||0)-(cash||0);
  const epsStart=prior3?valueAt(H.eps,prior3.year):null,fcfStart=prior3?.fcf;
  return{source:'SEC EDGAR companyfacts',fetched_at:new Date().toISOString(),cik,filing_date:rev.filed||null,period:year,years:annualRows.length,currency:'USD',is_etf:false,name:facts.entityName||hit.title||symbol,annual:annualRows,revenue,net_income:netIncome,eps_ttm:eps,bvps:shares&&equity?equity/shares:null,fcf_latest:fcf,fcf_base:annualRows.length>1&&annualRows.at(-2)?.fcf!=null?(fcf+annualRows.at(-2).fcf)/2:fcf,rev_cagr_3y:prior3?cagr(revenue,prior3.revenue,3):null,rev_cagr_5y:prior5?cagr(revenue,prior5.revenue,5):null,eps_cagr:prior3?cagr(eps,epsStart,3):null,fcf_cagr:prior3?cagr(fcf,fcfStart,3):null,gross_margin:ratio(gross,revenue),op_margin:ratio(opIncome,revenue),net_margin:ratio(netIncome,revenue),fcf_margin:ratio(fcf,revenue),roe:ratio(netIncome,equity),roa:ratio(netIncome,assets),roic:invested>0&&opIncome!=null?opIncome*(1-taxRate)/invested:null,debt_to_equity:ratio(debt,equity),net_cash:cash!=null&&debt!=null?cash-debt:null,total_debt:debt,cash,current_ratio:ratio(ca,cl),quick_ratio:ca!=null&&cl?((ca-(inv||0))/cl):null,interest_coverage:interest>0&&opIncome!=null?opIncome/interest:null,capex,ocf,tax_rate:taxRate,assets,liabilities,equity,shares,pe:null,forward_pe:null,pb:null,ps:null,peg:null,ev_ebitda:null,dividend_ps:null,buyback:null,attempts:[],note:'Official SEC EDGAR/XBRL filing facts. Values are filing-derived and carry a filing date; current market-price multiples are added separately when possible.'};
}
function withMarketPrice(f,price){if(!f||f.error)return f;const p=n(price);if(p==null||p<=0)return f;const out={...f};out.mcap=out.shares?p*out.shares:null;out.pe=out.eps_ttm>0?p/out.eps_ttm:null;out.pb=out.bvps>0?p/out.bvps:null;out.ps=out.mcap&&out.revenue?out.mcap/out.revenue:null;const growth=out.eps_cagr!=null?out.eps_cagr*100:null;out.peg=out.pe&&growth>0?out.pe/growth:null;return out;}
